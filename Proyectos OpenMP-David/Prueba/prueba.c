...
void main(int argc , char*argv[]){
  omp_set_num_threads(2); //Setea la cantidad de Threads a usar. No debe superar a la cantidad física de cores
  if (atoi(argv[1]) == 0) //Se controla el argumento del programa. Con : a.exe 0 => Secuencial ; a.exe 1 => Paralelo
    showTimeSeq();
  else
    showTimePrl();
}
